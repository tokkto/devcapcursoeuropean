...
describe('Test The GET Endpoints', () => {
  ...
  it('Test Expand Entity Endpoint', async () => {
    const { data } = await GET`/odata/V2/processor/Customers?$select=firstName&$expand=incidents`
...
describe('Draft Choreography APIs', () => {
  ...
  it('Create an incident ', async () => {
    const { status, statusText, data } = await POST(`/odata/V2/processor/Incidents`, {
  ...
  it('+ Activate the draft & check Urgency code as H using custom logic', async () => {
    const response = await POST(
      `/odata/V2/processor/Incidents(ID=${draftId},IsActiveEntity=false)/ProcessorService.draftActivate`
    )
  ...
  it('+ Test the incident status', async () => {
    const {
      status,
      data: { status_code, ID }
    } = await GET(`/odata/V2/processor/Incidents(ID=${draftId},IsActiveEntity=true)`)
    incidentId = ID
  ...
  describe('Close Incident and Open it again to check Custom logic', () => {
    it(`Should Close the Incident-${draftId}`, async () => {
      const { status } = await POST(
        `/odata/V2/processor/Incidents(ID=${incidentId},IsActiveEntity=true)/ProcessorService.draftEdit`,
        {
    ...
    it(`Should Close the Incident-${draftId}`, async () => {
      const { status } = await PATCH(`/odata/V2/processor/Incidents(ID=${incidentId},IsActiveEntity=false)`, {
        status_code: 'C'
      })
    ...
    it('+ Activate the draft & check Status code as C using custom logic', async () => {
      const response = await POST(
        `/odata/V2/processor/Incidents(ID=${incidentId},IsActiveEntity=false)/ProcessorService.draftActivate`
      )
    ...
    it('+ Test the incident status to be closed', async () => {
      const {
        status,
        data: { status_code }
      } = await GET(`/odata/V2/processor/Incidents(ID=${incidentId},IsActiveEntity=true)`)
      expect(status).to.eql(200)
  ...
  it('- Delete the Draft', async () => {
    const response = await DELETE(`/odata/V2/processor/Incidents(ID=${draftId},IsActiveEntity=false)`)
    expect(response.status).to.eql(204)
  })

  it('- Delete the Incident', async () => {
    const response = await DELETE(`/odata/V2/processor/Incidents(ID=${draftId},IsActiveEntity=true)`)
    expect(response.status).to.eql(204)
  })
})
